...
export class S3Service {
    ...
    public async downloadFile(bucketName: string, key: string): Promise<AWS.S3.Body>{
        const params = {
            Bucket: bucketName,
            Key: key
        };
        const fileObject: GetObjectOutput = await this.s3.getObject(params).promise();
        return fileObject.Body.toString()
    }

    public async listObjects(bucketName: string): Promise<AWS.S3.ListObjectsOutput>{
        const params = {
            Bucket: bucketName
        };
